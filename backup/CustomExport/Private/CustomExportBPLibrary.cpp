...
void UCustomExportBPLibrary::ExportCamera(const UCameraComponent* Component)
{
	...
	FString FByteStr = FString(ANSI_TO_TCHAR(cameraBytes));*/

	/*FString FByteStr = TEXT("wo men shi zhu");
	TArray<uint8> ArryBytes;
	ArryBytes.AddUninitialized(FByteStr.Len());
	StringToBytes(FByteStr, ArryBytes.GetData(), FByteStr.Len());

	FString CameraSaveBinaryFile = SavePath + CameraBinaryFileName;
	FFileHelper::SaveArrayToFile(ArryBytes, *CameraSaveBinaryFile);*/
	//delete[] cameraBytes;
